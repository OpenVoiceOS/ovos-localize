...
### Why does my language show "machine translation is not available" instead of an Auto-translate button?
The editor can offer an optional per-string auto-translate *suggestion* (a starting point a human then edits and submits). For some languages there is little to no machine-translation signal, so that suggestion would be unreliable — and worse, it risks anchoring a translator to a plausible-looking but wrong first draft instead of translating directly from the English source. For those "human-first" languages the suggestion button is hidden; the editor otherwise works exactly the same way. Kabyle (`kab`) is one such language. If good machine-translation coverage becomes available for a language, open an issue asking for it to be removed from `HUMAN_FIRST_LANGS`.
